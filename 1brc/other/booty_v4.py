"""
    Version differences from mine:
        - Uses mmap to read the file in chunks
        - Uses multiprocessing to process the chunks

    Execution Time (1b rows):
        - pypy:     44.166762957931496 = 44s
"""
...
def read_file_in_chunks(file_path):
    ...
    for loc, val in sorted(final.items()):
        print(f"{loc.decode()}={0.1*val[2]:.1f}/{(0.1*val[1] / val[0]):.1f}/{0.1*val[3]:.1f}")


if __name__ == "__main__":
    import sys
    read_file_in_chunks(file_path=sys.argv[1])